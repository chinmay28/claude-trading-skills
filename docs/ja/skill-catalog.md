...
| **[Morning Advisor]({{ '/ja/skills/morning-advisor/' | relative_url }})** | cron駆動の毎日オーケストレーター（レジーム判定 → ポジション管理 → スクリーナー → オプション/株式選択 → Alpaca paperへ自動執行）。ロングコール/プット/株式のみ、保有期間1週間〜1ヶ月、1取引リスク ≤ 1%。S&P 500比のアルファを `state/benchmark_log.jsonl` に記録 | <span class="badge badge-api">FMP + Alpaca必須</span> |
...
- **[Morning Advisor]({{ '/ja/skills/morning-advisor/' | relative_url }})** - cron駆動の毎日オーケストレーター。0〜3トレード（ロングコール/プット/株式）を選定し、Alpaca paper に自動執行、S&P 500 比のアルファを記録
